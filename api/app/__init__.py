...
def create_app():
    ...
    CORS(app, resources={r"/v4-api/*": {"origins": "*"}})
    
    # Initialize DB indexes
    init_db()
    
    # Register Blueprints
    app.register_blueprint(auth_bp, url_prefix='/v4-api')
    app.register_blueprint(friends_bp, url_prefix='/v4-api/friends')
    app.register_blueprint(media_bp, url_prefix='/v4-api/media')
    app.register_blueprint(status_bp, url_prefix='/v4-api/status')
    app.register_blueprint(messages_bp, url_prefix='/v4-api/messages')
    app.register_blueprint(calls_bp, url_prefix='/v4-api/calls')
    
    @app.route('/v4-api/health')
    def health():
